...
def final_result():
    ...
    temp = [round(float(x),5) for x in temp]
    print(f"The Sum of all probabilities : {temp} = {round(sum(temp),3)}")     #sum of P(a,b,c,...,n)=1
    return temp


def pie_graph(scores):
    plt.pie(scores, labels=scores)
    plt.legend(unique[c-1], title="Scores", loc="best")
    plt.title(f"Scores for Probability of {data.columns[c - 1]}")
    plt.show()
